...
class ASSET_SKETCHER_OT_GenerateAssetPreview(bpy.types.Operator):
    ...
    def convert_group_instance(self,context,scene,obj):
        ...
        scene.view_layers[0].objects.active = context.selected_objects[0]
        bpy.ops.object.join()
    ...
    def arrange_objects(self,context,preview_scene,objs):
        ...
        for i,obj in enumerate(objs):
            ...
            if linked_obj.type == "EMPTY":
                ...
                preview_scene.view_layers[0].objects.active = linked_obj
    ...
    def execute(self, context):
        ...
        preview_scene.render.filter_size = 1.5
        ...
        if "preview_cam_empty" not in preview_scene.objects:
            ...
            preview_scene.collection.objects.link(preview_cam_empty)
        preview_cam_empty = bpy.data.objects["preview_cam_empty"]
        ...
        if "preview_cam" not in preview_scene.objects:
            ...
            preview_scene.collection.objects.link(preview_cam)
        ...
        if "preview_light" not in preview_scene.objects:
            if "preview_light" not in bpy.data.lights:
                bpy.data.lights.new("preview_light","SUN")
            light_data = bpy.data.lights["preview_light"]
            ...
            preview_scene.collection.objects.link(preview_light)
        ...
        for i,asset in enumerate(wm.asset_sketcher.asset_list):
            ...
            if asset.type == "OBJECT":
                ...
                for obj in objs:
                    linked_obj = obj.copy()
                    linked_obj.data = obj.data.copy()
                    preview_scene.collection.objects.link(linked_obj)
                    linked_obj.select_set(True)
                    preview_scene.view_layers[0].objects.active = linked_obj
                bpy.ops.object.join()
                ...
                instance = obj.copy()
                preview_scene.collection.objects.link(instance)
                instance.select_set(True)
                preview_scene.view_layers[0].objects.active = instance
            ...
            preview_scene.collection.objects.unlink(linked_obj)
