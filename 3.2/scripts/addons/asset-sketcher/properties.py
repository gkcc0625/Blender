...
class ASSET_SKETCHER_ItemObjects(bpy.types.PropertyGroup):
    name: StringProperty(default="")


class ASSET_SKETCHER_AssetCategories(bpy.types.PropertyGroup):
    name: StringProperty(default="")
...
class ASSET_SKETCHER_CanvasList(bpy.types.PropertyGroup):
    name: StringProperty(default="")
    canvas_item: PointerProperty(type=bpy.types.Object)


### This Class stores all asset settings for each entry
class ASSET_SKETCHER_AssetList(bpy.types.PropertyGroup):
    name: StringProperty(default="")
    make_single_user: BoolProperty(description="Objects gets added as single User Obejcts with its own Mesh Data.",
                                    default=False)
    canvas: BoolProperty(description="Added object functions also as Canvas and can be painted on.", default=False)

    categories: CollectionProperty(type=ASSET_SKETCHER_AssetCategories)

    group_mode: EnumProperty(description="Defines how Assets are picked from Group. Random or Cyclic.",
                              items=(("RANDOM", "Pick Random", "Random"), ("CYCLIC", "Pick Cyclic", "Cyclic")))
    asset_distance_mode: EnumProperty(description="Defines if Assets are placed by distance or fixed count per line.",
                                       items=(("DISTANCE", "Distance", "Distance"), ("FIXED", "Fixed", "Fixed")))
    group_objects: CollectionProperty(type=ASSET_SKETCHER_ItemObjects)

    asset_distance: FloatProperty(description="Set the asset distance for each Paint Stroke.", default=1.0, min=0.0,
                                   max=30.0)
    asset_count: IntProperty(description="Sets the amount of assets that are going to be placed for each Line Stroke.",
                              default=2, min=1)
    distance_constraint: BoolProperty(description="Constraint Distance to Brush Radius.", default=False)
    z_offset: FloatProperty(description="Set the Z offset the Assets gets placed from the underground.", default=0.0,
                             min=-30.0, max=30.0)
    slope_threshold: FloatProperty(description="Defines the threshold if assets can be placed on a slope.",
                                    default=0.0, min=0.0, max=1.0, subtype="FACTOR")
    surface_orient: FloatProperty(description="Defines how much the asset is aligned to the surface normal.",
    ...
    rand_rot_x: FloatProperty(description="Set the random rotation of a placed asset.", default=0, subtype='ANGLE',
                               min=0.0, max=math.radians(360))
    rand_rot_y: FloatProperty(description="Set the random rotation of a placed asset.", default=0, subtype='ANGLE',
                               min=0.0, max=math.radians(360))
    rand_rot_z: FloatProperty(description="Set the random rotation of a placed asset.", default=0, subtype='ANGLE',
                               min=0.0, max=math.radians(360))
    ...
    rot_x: FloatProperty(description="Set the rotation of a placed asset.", default=0, subtype='ANGLE',
                          min=math.radians(-180), max=math.radians(180))
    rot_y: FloatProperty(description="Set the rotation of a placed asset.", default=0, subtype='ANGLE',
                          min=math.radians(-180), max=math.radians(180))
    rot_z: FloatProperty(description="Set the rotation of a placed asset.", default=0, subtype='ANGLE',
                          min=math.radians(-180), max=math.radians(180))
    ...
    type: EnumProperty(description="", default="OBJECT", items=(
    ("OBJECT", "Object", "Object", "", 0), ("GROUP", "Group", "Group", "", 1), ("EMPTY", "Empty", "Empty", "", 2)))
...
class ASSET_SKETCHER_MergeObjectAvailableObjects(bpy.types.PropertyGroup):
    name: StringProperty(default="")


### All Addon Properties are stored in this Group
class ASSET_SKETCHER_AssetSketcherProperties(bpy.types.PropertyGroup):
    asset_preview: BoolProperty(
        description="Preview Asset. Does not work well with Blenders undo system. May cause crashes. Turned off by default.",
        default=False)
    ...
    sketch_mode_items = []
    sketch_mode_items.append(("PAINT", "Paint Mode", "Paint mode", "BRUSH_DATA", 0))
    sketch_mode_items.append(("SCALE", "Scale Mode", "Scale mode", "FULLSCREEN_ENTER", 1))
    sketch_mode_items.append(("GRID", "Grid Mode", "Grid mode", "GRID", 2))
    sketch_mode_items.append(("LINE", "Line Mode", "Line mode", "LINE_DATA", 3))

    sketch_mode: EnumProperty(name="Asset Sketcher Mode", description="Asset Sketcher Painting Mode", default="PAINT",
                               items=sketch_mode_items)
    sketch_plane_axis: EnumProperty(name="Sketch Plane Axis", description="The Plane the grid system is in.",
                                     default="XY", items=(
        ("XY", "XY Plane", "XY Plane", "", 0), ("XZ", "XZ Plane", "XZ Plane", "", 1),
        ("YZ", "YZ Plane", "YZ Plane", "", 2)))
    ...
    visible_objects_collection: CollectionProperty(type=ASSET_SKETCHER_MergeObjectAvailableObjects)
    merge_object: StringProperty(
        description="If a Name is set, all new Assets will be merged into one Object. Increases performance. Delete Brush does not work on single Assets anymore.",
        default="")
    ...
    canvas_list: CollectionProperty(type=ASSET_SKETCHER_CanvasList)
    canvas_list_index: IntProperty(default=0)

    ### asset list
    asset_list: CollectionProperty(type=ASSET_SKETCHER_AssetList)
    asset_list_index: IntProperty(default=0)

    display_asset_list: CollectionProperty(type=ASSET_SKETCHER_AssetList)
    display_asset_list_index: IntProperty(default=0, update=update_asset_list_index)

    categories: CollectionProperty(name="Asset Categories", type=ASSET_SKETCHER_AssetCategories)
    categories_enum: EnumProperty(name="Asset Categories",
                                   description="Create Asset Categories to better organize your Assets.",
                                   items=get_categories, update=update_category_display)
...
def register_properties():
    bpy.types.WindowManager.asset_sketcher = PointerProperty(type=ASSET_SKETCHER_AssetSketcherProperties)
    bpy.types.Scene.asset_sketcher = PointerProperty(type=ASSET_SKETCHER_AssetSketcherProperties)
