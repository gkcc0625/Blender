...
class UIMainModule(Panel):
    ...
    def draw(self, context):
        ...
        layout.operator('view3d.ke_get_set_material', icon="MOUSE_MOVE", text="Get&Set Material")

        layout.operator('object.ke_local_by_distance')
        row = layout.row(align=True)
        row.operator('view3d.ke_zerolocal').mode = 0
        row.operator('view3d.ke_zerolocal', text="Zero Local R").mode = 1
        layout.operator('view3d.ke_unhide_or_local', icon="HIDE_OFF")
...
class KeZeroLocal(Operator):
    ...
    mode: bpy.props.IntProperty(name="Mode", default=0, options={"HIDDEN"})
