...
import bmesh
from ._utils import average_vector, mouse_raycast, tri_points_order, correct_normal, rotation_from_vector, get_islands
...
def register():
    if bpy.context.preferences.addons[__package__].preferences.m_unrotator:
        try:
            if not bpy.context.preferences.addons[__package__].preferences.m_modeling:
                UIUnrotatorModule.bl_category = __package__
                UIUnrotatorModule.bl_parent_id = ""
            else:
                UIUnrotatorModule.bl_category = ""
                UIUnrotatorModule.bl_parent_id = "UI_PT_M_MODELING"
...
def unregister():
    if "bl_rna" in UIUnrotatorModule.__dict__:
