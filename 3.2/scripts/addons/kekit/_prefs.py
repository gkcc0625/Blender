...
class kekit_properties(PropertyGroup):
    ...
    selmode_mouse : BoolProperty(description="LEGACY FEATURE: Mouse-Over Element Mode Selection for old Sel.mode script", default=v["selmode_mouse"])
...
class VIEW3D_OT_ke_prefs_save(Operator):
    ...
    bl_label = "Save keKit Settings (& keymappings)"
    bl_description = "Saves current kit settings, and key mappings. (JSON-files, settings are global & persistent)\n" \
                     "Import key mappings in kekit add-on prefs."
...
class kekit_addon_preferences(AddonPreferences):
    ...
    prefs_loc : StringProperty(name="Prefs File Location", default="",
                               description="Note: This cannot be changed - Field intended for clipboard copy-paste")
    ui_scale : FloatProperty(name="Modal UI Text Scale", default=1.0, min=0.1, max=10,
    ...
    def draw(self, context):
        ...
        self.prefs_loc = utils.script_path_user()
        layout = self.layout

        layout.operator("view3d.ke_prefs_save", icon="FILE_CACHE", text="Save keKit Settings")
        row = layout.row()
        row.operator("ke.kit_keymap", icon="COPYDOWN", text="Export keKit Keymapping").mode = "SAVE"
        row.operator("ke.kit_keymap", icon="PASTEDOWN", text="Import keKit Keymapping").mode = "LOAD"
        layout.label(text="keKit Prefs & Keymap Location:")
        layout.prop(self, "prefs_loc", text="")
        row = layout.row(align=True)
        row.label(text="Viewport TT-Toggle Icons:")
        row.prop(context.scene.kekit, "tt_hide", text="Hide", toggle=True)

        # MODAL OPS COLOURS
        layout.label(text="Modal Preferences:")
        row = layout.column(align=True)
        row.use_property_split = True
        row.prop(self, "ui_scale")
        ...
        layout.label(text="Assigned keKit Shortcuts:")

        for km in wm.keyconfigs.user.keymaps:
            used_cat = False
            # Check if valid category (todo: better method)
            for ki in km.keymap_items:

                if ki.idname in keops:
                    used_cat = True
                    break
                elif km.name == "Window":
                    if ki.idname == "wm.call_menu_pie":
                        if ki.properties:
                            val = []
                            for value in ki.properties.values():
                                val.append(value)
                            if val[0] in ke_wm_pies:
                                used_cat = True
                                break
            ...
            if used_cat:
                ...
                if km.show_expanded_items:
                    ...
                    for ki in km.keymap_items:
                        ...
                        if ki.idname in keops or wm_case:
                            rna_keymap_ui.draw_kmi(["ADDON", "USER", "DEFAULT"], kc, km, ki, col, 0)
