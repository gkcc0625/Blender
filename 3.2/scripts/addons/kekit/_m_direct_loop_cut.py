...
class UIDirectLoopCutModule(Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.prop(context.scene.kekit, "dlc_so")
        if not context.scene.kekit.dlc_so:
...
def register():
    if bpy.context.preferences.addons[__package__].preferences.m_directloopcut:
        try:
            if not bpy.context.preferences.addons[__package__].preferences.m_modeling:
                UIDirectLoopCutModule.bl_category = __package__
                UIDirectLoopCutModule.bl_parent_id = ""
            else:
                UIDirectLoopCutModule.bl_category = ""
                UIDirectLoopCutModule.bl_parent_id = "UI_PT_M_MODELING"
...
def unregister():
    if "bl_rna" in UIDirectLoopCutModule.__dict__:
