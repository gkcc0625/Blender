...
def apply_transform(obj=None, use_loc=False, use_rot=False, use_scl=False):
    mb = obj.matrix_basis
    ident_mtx = Matrix()
    loc, rot, scl = mb.decompose()

    # rotation
    tm = Matrix.Translation(loc)
    rm = mb.to_3x3().normalized().to_4x4()
    sm = Matrix.Diagonal(scl).to_4x4()

    transform = [ident_mtx, ident_mtx, ident_mtx]
    basis = [tm, rm, sm]
    ...
    mtx = transform[0] @ transform[1] @ transform[2]
    if hasattr(obj.data, "transform"):
        obj.data.transform(mtx)
    for c in obj.children:
        c.matrix_local = mtx @ c.matrix_local
...
def get_ke_operators():
    return ["view3d.ke_bg_sync",
            "view3d.ke_clean",
            ...
            "mesh.ke_maya_connect",
            "mesh.ke_triple_connect_spin",
            "view3d.ke_copyplus",
            "view3d.cursor_fit_selected_and_orient",
...
def get_ke_wm_piemenus():
    return ["VIEW3D_MT_ke_pie_subd",
            "VIEW3D_MT_ke_pie_fitprim",
            "VIEW3D_MT_ke_pie_fitprim_add",
            "VIEW3D_MT_ke_pie_align",
            "VIEW3D_MT_ke_pie_snapping",
            "VIEW3D_MT_ke_pie_fit2grid",
            "VIEW3D_MT_ke_pie_fit2grid_micro",
