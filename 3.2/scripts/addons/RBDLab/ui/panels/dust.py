...
from ...core.functions import have_particle_sytem_dust, get_first_mesh_visible, get_constraints_from_obj


class PARTICLES_PT_dust(Panel):
    bl_label = "Dust"
    bl_idname = "PARTICLES_PT_dust"
    ...
    def draw(self, context):
        ...
        if not have_particle_sytem_dust():
            obj = get_first_mesh_visible()
            if obj:
                const = get_constraints_from_obj(obj)
                if const:
                    col.prop(context.scene.rbdlab_props, 'ps_dust_from')
            col.operator("dust.add", text="Emit From Current Frame")
        else:
            ...
            col.operator("dust.rm", text="Remove Dust")