...
def create_rotation_matrix_from_normal(obj, normal, location=Vector((0, 0, 0)), debug=False):
    ...
    if debug:
        ...
        draw_vector(objup, objloc, color=(1, 0, 0), modal=False)
        draw_vector(normal, location, modal=False)
        draw_vector(tangent, location, color=(0, 1, 0), modal=False)
        draw_vector(binormal, location, color=(0, 0, 1), modal=False)
