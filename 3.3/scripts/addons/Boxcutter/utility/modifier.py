...
def sort(obj, option=None, ignore=[], sort_types=sort_types, last_types=[], first=False, static_sort=False, ignore_hidden=True, sort_depth=0, ignore_flag=' ', stop_flag='_', use_index_operator=True):
    modifiers = []
    ...
    for type in sort_types:
        ...
        for mod in reversed(sortable):
            ...
            if mod.type not in sort_types or mod in ignore or not visible or mod.name[0] == ignore_flag:
                continue
    ...
    for index, mod in enumerate(modifiers):
        ...
        if first:
            move_to_index(m, index=index, rebuild=not use_index_operator)
...
def stored(mod):
    exclude = {'__doc__', '__module__', '__slots__', '_RNA_UI', 'bl_rna', 'rna_type', 'face_count', 'is_override_data', 'particle_system', 'map_curve'}
...
def new(obj, name=str(), _type='BEVEL', mod=None, props={}):
    ...
    if mod:
        ...
        for pointer in dir(mod):
            if '__' in pointer or pointer in {'bl_rna', 'rna_type', 'type', 'face_count', 'falloff_curve', 'vertex_indices', 'vertex_indices_set', 'is_override_data', 'particle_system', 'map_curve'}:
                continue
