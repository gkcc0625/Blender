...
class ASSET_SKETCHER_OT_SketchAssets(bpy.types.Operator):
    ...
    def __init__(self):
        ...
        self.projected_mouse_always = Vector((0,0,0))
        ...
        self.last_valid_ground_normal = None
        ...
        self.sketch_mode = ["PAINT","SCALE","GRID","LINE"]
    ...
    def invoke(self, context, event):
        ...
        elif wm.asset_sketcher.sketch_mode == "LINE":
            self.sketch_mode_idx = 3
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            pie_shortcut = get_addon_prefs(context).paint_mode_shortcut
            asset_list_shortcut = get_addon_prefs(context).asset_list_shortcut
            if event.type == asset_list_shortcut and event.value == "PRESS" and not event.shift and not event.ctrl and not event.alt:
                bpy.ops.asset_sketcher.asset_list_popup("INVOKE_DEFAULT")
                return {"RUNNING_MODAL"}
            if event.type == pie_shortcut and event.value == "PRESS" and not event.shift and not event.ctrl and not event.alt:
                bpy.ops.wm.call_menu_pie(name="ASSET_SKETCHER_MT_menu")
                return{"RUNNING_MODAL"}
            ...
            if len(self.ray_data_list) > 0:
                for ray_data in self.ray_data_list:
                    self.canvas_found = ("as_asset" not in ray_data[1] and not wm.asset_sketcher.use_canvas_objects) or (ray_data[1] in canvas_objects and wm.asset_sketcher.use_canvas_objects) or (wm.asset_sketcher.canvas_all and len(self.ray_data_list) > 0)
            ...
            if wm.asset_sketcher.sketch_mode == "PAINT":
                ...
                self.line_mode_handling(context, event)
            elif wm.asset_sketcher.sketch_mode == "SCALE":
    ...
    def draw_callback_px(self,context,event):
        wm = context.window_manager
        if ((self.mouse_on_canvas and self.in_view_3d and wm.asset_sketcher.sketch_mode in ["PAINT","GRID","LINE"]) or self.f_key or event.ctrl) and not self.scale_stroke and not event.alt:
    ...
    def transform_object(self,context,event,obj,custom_direction=None,custom_ground_normal=None,custom_location=None,custom_scale=None,custom_z_offset=None,use_projection_offset=True,no_random_rot=False):
        ...
        if wm.asset_sketcher.sketch_mode in ["PAINT","SCALE","LINE"]:
    ...
    def set_brush_size(self,context,event):
        ...
        if self.f_key:
            changed_brush_size = (self.projected_mouse_always - self.stroke_start_pos).length*2
            if (self.stroke_start_pos2d - Vector((event.mouse_region_x, event.mouse_region_y)))[0] > 0:
                wm.asset_sketcher.brush_size = self.brush_size_tmp - changed_brush_size
            else:
                wm.asset_sketcher.brush_size = self.brush_size_tmp + changed_brush_size
    ...
    def set_brush_density(self, context, event):
        ...
        if self.f_key_shift:
            changed_brush_density = (self.projected_mouse_always - self.stroke_start_pos).length * 2 * 0.7
            if (self.stroke_start_pos2d - Vector((event.mouse_region_x, event.mouse_region_y)))[0] > 0:
                wm.asset_sketcher.brush_density = self.brush_density_tmp - changed_brush_density
            else:
                wm.asset_sketcher.brush_density = self.brush_density_tmp + changed_brush_density
    ...
    def update_ray_data_with_position(self, context, event, depth=-1, custom_position=None):
        ...
        if wm.asset_sketcher.sketch_mode in ["SCALE", "PAINT", "LINE"]:
            ...
            if (not self.mouse_on_canvas) or self.scale_stroke:
                ...
                if view_ray != None:
                    ...
                    if point_on_plane != None and not self.f_key:
                        self.projected_mouse = point_on_plane
                    if point_on_plane != None:
                        self.projected_mouse_always = point_on_plane
        ...
        if self.asset_preview == None and wm.asset_sketcher.asset_preview:
            ...
            if self.asset_item.type == "EMPTY":
                if self.asset_item.name not in bpy.data.objects:
                    return
                for obj in bpy.data.objects[self.asset_item.name].instance_collection.objects:
                    if obj.parent == None:

                        reference_obj = obj
                        self.asset_preview = bpy.data.objects[ self.instance_object(reference_obj,reference_obj,self.asset_item.name,True) ]
                        if self.asset_preview == None:
                            return
                        self.asset_preview.select_set(True)
                        context.scene.cursor.location = self.asset_preview.location - obj.location
                        for child in self.asset_preview.children:
                            child.select_set(True)
                        context.view_layer.objects.active = self.asset_preview
            ...
            self.asset_preview.select_set(False)
            ...
            if ((self.scale_stroke or self.brush_stroke) and not self.asset_preview.hide_viewport) or event.ctrl or event.alt or not self.in_view_3d:
                self.asset_preview.hide_viewport = True
            elif not self.scale_stroke and not self.brush_stroke and self.asset_preview.hide_viewport:
                self.asset_preview.hide_viewport = False
    ...
    def delete_asset_handling(self,context,event):
        ...
        if ((event.type == "LEFTMOUSE" and event.value == "PRESS") or event.type == "MOUSEMOVE") and self.delete_stroke and event.ctrl:
            ...
            for i,ray_data in enumerate(self.ray_data_list):
                obj = ray_data[1]
                if wm.asset_sketcher.sketch_mode in ["SCALE","PAINT","LINE"] and not "as_merge_object" in obj:
                    if i == 0:
                        if "as_asset" in obj and "as_instance_collection" not in obj and "as_preview" not in obj:
                            if (wm.asset_sketcher.delete_only_selected and obj.select_get()) or not wm.asset_sketcher.delete_only_selected:
            ...
            for ray_data in self.ray_data_list:
                if "as_asset" in ray_data[1]:
                    ...
                    if "as_asset_name" in obj:
                        ...
                        asset_name = obj.name
                    elif "as_instance_collection" in obj:
                        asset_name = obj["as_instance_collection"]
    ...
    def calc_average_object_scale(self,obj):
        if obj.type == "MESH":
            ...
            if len(obj.instance_collection.objects) > 0:
                average = 0
                for i,obj in enumerate(obj.instance_collection.objects):
    ...
    def scale_mode_handling(self,context,event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS" and self.in_view_3d and self.mouse_on_canvas and not event.ctrl and not self.f_key and not self.f_key_shift and not event.alt:
            ...
            new_asset =  bpy.data.objects[ self.instance_object(reference_obj,reference_obj,self.asset_item.name,self.asset_item.make_single_user) ]
    ...
    def grid_mode_handling(self,context,event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS" and self.in_view_3d and not event.alt:
            ...
            if self.mouse_on_canvas:
                ...
                self.stroke_start_pos = Vector((1000,1000,1000))
    ...
    def get_random_point_in_circle(self, circle_center, circle_normal, diameter):
        rot_mat = circle_normal.rotation_difference(Vector((0, 0, 1))).to_matrix().to_3x3()
        # calc random distribution within brush circle
        angle = 2 * math.pi * random.random()
        radius = diameter * math.sqrt(random.random()) * .5

        random_x = radius * math.cos(angle)
        random_y = radius * math.sin(angle)
        random_z = 0

        # rotate point within ground normal
        p = Vector((random_x, random_y, random_z))
        p = rot_mat @ p
        p += circle_center
        return p

    def get_project_point(self, context, wm, point, projection_normal):
        view_distance = context.space_data.region_3d.view_distance
        start = point + (projection_normal * view_distance*.7)
        end = point - (projection_normal * view_distance*.7)
        ...
        for i, ray_data in enumerate(p_ray_data):
            if (point - ray_data[3]).length < p_ray_distance:
                index = i
                p_ray_distance = (point - ray_data[3]).length
        return p_ray_data, index

    def create_asset_stroke(self, context, event, wm, stroke_start_pos, stroke_direction, stroke_distance, stroke_length, stroke_assets, projected_mouse_snapped, asset_count):
        stroke_samples = int(stroke_length / stroke_distance)
        if self.asset_item.asset_distance_mode == "FIXED" and wm.asset_sketcher.sketch_mode == "LINE":
            stroke_distance = stroke_length / max(1,(self.asset_item.asset_count-1))
            stroke_samples = self.asset_item.asset_count

        stroke_inbetween_points = []

        if hasattr(context.space_data, "region_3d"):
            _3d_to_2d = bpy_extras.view3d_utils.location_3d_to_region_2d
            _2d_to_vec_3d = bpy_extras.view3d_utils.region_2d_to_vector_3d
            rv3d = context.space_data.region_3d
            region = context.region
            view_vec = Vector((0, 0, 1))
            view_vec.rotate(rv3d.view_rotation)

        for i in range(stroke_samples):
            stroke_inbetween_point = stroke_start_pos + (stroke_direction * stroke_distance * i)
            stroke_inbetween_points.append(stroke_inbetween_point)
        for j, point in enumerate(stroke_inbetween_points):
            for i in range(wm.asset_sketcher.brush_density):
                final_p = self.get_random_point_in_circle(point, self.ground_normal, wm.asset_sketcher.brush_size)
                # p_ray_data, index = self.get_project_point(context, wm, final_p, self.ground_normal)

                final_p_2d = _3d_to_2d(region, rv3d, final_p)
                view_vec_perspective = _2d_to_vec_3d(region,rv3d, final_p_2d)

                p_ray_data, index = self.get_project_point(context, wm, final_p, -view_vec_perspective )

                if len(p_ray_data) > 0:
                    if p_ray_data[index][1].name in wm.asset_sketcher.canvas_list or not wm.asset_sketcher.use_canvas_objects:
                        ...
                        hit_distance_in_threshold = (hit_position - final_p).length <= min(wm.asset_sketcher.brush_size * .5, 10)
                        ...
                        backface_normals = ground_normal.dot(view_vec)
                        if slope >= self.asset_item.slope_threshold:

                            # place new asset
                            if self.asset_item.stroke_orient:
                                dir = stroke_direction  # (self.projected_mouse - hit_position).normalized()
                            else:
                                dir = Vector((0, 1, 0))

                            new_asset = bpy.data.objects[
                                self.instance_object(reference_obj, reference_obj, self.asset_item.name,self.asset_item.make_single_user)]
                            self.stroke_assets.append(new_asset)
                            ...
                            self.projected_mouse_last = Vector(projected_mouse_snapped)
            asset_count += 1
        stroke_start_pos = projected_mouse_snapped

        return {"stroke_start_pos": stroke_start_pos, "asset_count": asset_count, "stroke_assets": stroke_assets}

    def merge_stroke_assets(self, context, wm, stroke_assets):
        if len(stroke_assets) > 0 and context.scene.asset_sketcher.merge_object != "":
            assets = self.convert_group_instance(context, wm, self.stroke_assets)
            self.merge_asset(context, wm, assets)

    def line_mode_handling(self, context, event):
        wm = context.window_manager
        ### handle brush input
        self.brush_stroke_hist = bool(self.brush_stroke)

        modifier_keys_active = event.ctrl or self.f_key or self.f_key_shift or event.alt
        if event.type == "LEFTMOUSE" and event.value == "PRESS" and self.in_view_3d and not event.alt and self.canvas_found:
            if not modifier_keys_active:
                self.brush_stroke = True
        elif (event.type == "LEFTMOUSE" and event.value == "RELEASE" and self.brush_stroke) or modifier_keys_active:
            self.brush_stroke = False

        # line stroke begins
        if self.brush_stroke and self.brush_stroke_hist != self.brush_stroke:
            self.stroke_start_pos = Vector(self.projected_mouse)

        # line stroke is performed
        elif self.brush_stroke and self.brush_stroke:
            self.stroke_direction = (self.projected_mouse - self.stroke_start_pos).normalized()
            self.stroke_length = snap_value((self.stroke_start_pos - self.projected_mouse).length, self.stroke_distance)
            self.projected_mouse_snapped = self.stroke_start_pos + self.stroke_direction * self.stroke_length

            if event.type == "WHEELUPMOUSE":
                self.asset_item.asset_count += 1
            elif event.type == "WHEELDOWNMOUSE":
                self.asset_item.asset_count -= 1

        # line stroke ends
        elif not self.brush_stroke and self.brush_stroke_hist != self.brush_stroke:
            self.stroke_direction = (self.projected_mouse - self.stroke_start_pos).normalized()
            self.stroke_length = (self.stroke_start_pos - self.projected_mouse).length
            if wm.asset_sketcher.sketch_mode == "PAINT" or wm.asset_sketcher.sketch_mode == "LINE" and self.asset_item.asset_distance_mode == "DISTANCE":
                self.stroke_length = snap_value(self.stroke_length, self.stroke_distance) + self.stroke_distance
            elif self.asset_item.asset_distance_mode == "FIXED" and wm.asset_sketcher.sketch_mode == "LINE":
                self.stroke_length = self.stroke_length
            self.projected_mouse_snapped = self.stroke_start_pos + self.stroke_direction * self.stroke_length

            stroke_data = self.create_asset_stroke(context, event, wm, self.stroke_start_pos, self.stroke_direction,self.stroke_distance, self.stroke_length, self.stroke_assets, self.projected_mouse_snapped, self.asset_count)
            self.stroke_assets = stroke_data["stroke_assets"]
            self.merge_stroke_assets(context, wm, self.stroke_assets)

            bpy.ops.ed.undo_push(message="Line Stroke")
    ...
    def paint_mode_handling(self, context, event):
        ...
        # brush stroke is operated
        elif self.brush_stroke:
            self.stroke_direction = (self.projected_mouse - self.stroke_start_pos).normalized()
            self.stroke_length = snap_value((self.stroke_start_pos - self.projected_mouse).length,
                                            self.stroke_distance)
            self.projected_mouse_snapped = self.stroke_start_pos + self.stroke_direction * self.stroke_length

            # reset stroke start pos to current snapped mouse pos
            if (self.stroke_length >= self.stroke_distance):
                stroke_data = self.create_asset_stroke(context, event, wm, self.stroke_start_pos, self.stroke_direction, self.stroke_distance,self.stroke_length, self.stroke_assets,self.projected_mouse_snapped, self.asset_count)
                self.stroke_start_pos = stroke_data["stroke_start_pos"]
                self.stroke_assets = stroke_data["stroke_assets"]
                self.asset_count = stroke_data["asset_count"]
        ...
        # brush stroke ends
            ...
            if self.asset_count == 0:
                ...
                reference_obj = bpy.data.objects[asset_name]
                new_asset =  bpy.data.objects[ self.instance_object(reference_obj, reference_obj, self.asset_item.name,self.asset_item.make_single_user) ]
                self.transform_object(context, event, new_asset, custom_location=self.stroke_start_pos, custom_ground_normal=self.ground_normal, custom_direction=Vector((0,1,0)))
                self.stroke_assets.append(new_asset)
                self.asset_count += 1
