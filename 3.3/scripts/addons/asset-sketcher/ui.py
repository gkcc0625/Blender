...
class ASSET_SKETCHER_OT_NewMergeObject(bpy.types.Operator):
    ...
    def create_new_merge_object(self,context,wm):
        ...
        context.collection.objects.link(merge_object)
...
class ASSET_SKETCHER_OT_MergeObjects(bpy.types.Operator):
...
class ASSET_SKETCHER_PT_AssetSketcherUI(bpy.types.Panel):
    bl_idname = "ASSET_SKETCHER_PT_ui"
    bl_label = "Asset Sketcher 2.0.4"
...
#                    col.prop(wm.asset_sketcher,'asset_preview',text="Asset Preview")
            ...
            if context.scene.asset_sketcher.merge_object == "":
                subrow.operator("asset_sketcher.new_merge_object",text="",icon="ADD")
        ...
        if wm.asset_sketcher.box_asset_settings:
            ...
            row.operator("asset_sketcher.add_asset_category", text="",icon="ADD")
            row.operator("asset_sketcher.remove_asset_category", text="",icon="REMOVE")
            ...
            if addon_prefs.enable_asset_preview:
                ...
                if wm.asset_sketcher.asset_preview:
                    subcol.prop(wm.asset_sketcher,'asset_preview',text="",icon="HIDE_OFF")
                else:
                    subcol.prop(wm.asset_sketcher,'asset_preview',text="",icon="HIDE_ON")
            else:
                subcol.scale_y = 3
            ...
            col.template_list("ASSET_SKETCHER_UL_ListAssetItem","dummy",wm.asset_sketcher, "display_asset_list", wm.asset_sketcher, "display_asset_list_index", rows=5)
            ...
            if len(asset_list) > 0:
                ...
                if asset_sketcher.sketch_mode in ["PAINT","SCALE","LINE"]:
                    
                    row = col.row(align=True)

                    if wm.asset_sketcher.sketch_mode in ["LINE"]:
                        row.prop(asset_item, "asset_distance_mode", expand=True)
                        row = col.row(align=True)

                    if wm.asset_sketcher.sketch_mode in ["LINE"] and asset_item.asset_distance_mode == "FIXED":
                        row.prop(asset_item,"asset_count",text="Asset Count")
                    else:
        ...
        if wm.asset_sketcher.box_canvas_objects:
            ...
            col.template_list("ASSET_SKETCHER_UL_ListCanvasItem","dummy",wm.asset_sketcher, "canvas_list", wm.asset_sketcher, "canvas_list_index", rows=5)
        else:
            col = box.column()
            row = col.row()
...
class ASSET_SKETCHER_UL_ListCanvasItem(bpy.types.UIList):
...
class ASSET_SKETCHER_UL_ListAssetItem(bpy.types.UIList):
    def __init__(self):
        self.ico = ''
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ob = data
        ...
        if item.hide:
            row.prop(item,"hide",text="",icon="HIDE_ON",emboss=False)
        else:
