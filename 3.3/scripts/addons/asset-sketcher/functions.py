...
def ray_cast_any_version(start,end):
    ...
    if b_version_smaller_than((2,91,0)):
        ray_data = bpy.context.scene.ray_cast(view_layer=context.view_layer, origin=start, direction=delta.normalized(), distance=delta.length)
    else:
        ray_data = bpy.context.scene.ray_cast(depsgraph=context.view_layer.depsgraph, origin=start, direction=delta.normalized(), distance=delta.length)
