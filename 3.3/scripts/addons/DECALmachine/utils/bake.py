...
def setup_baking(bakescene, baketype, margin=3, to_active=True, ray_distance=0):
    if baketype == 'NORMAL':
...
def bake_target_mask(bakescene, bakebasepath, targetname, bakename, bakeimg, bakemat, margin=3, ray_distance=0):
    setup_baking(bakescene, 'EMIT', margin=margin, to_active=False, ray_distance=ray_distance)
...
def bake_decals_margin_mask(bakescene, bakebasepath, targetname, bakename, bakeimg, decals, margin=3, ray_distance=0):
    setup_baking(bakescene, 'EMIT', margin=margin, ray_distance=ray_distance)
...
def bake(bakescene, bakebasepath, decals, targetname, baketype, bakeimg, margin=3, ray_distance=0):
    if baketype in ['NORMAL']:
        setup_baking(bakescene, baketype, margin=margin, ray_distance=ray_distance)
    ...
    def match_pbr_nodes(mat, original_pbrnode, preview_pbrnode):
        for i, pi in zip(original_pbrnode.inputs, preview_pbrnode.inputs):
            ...
            if pi.name == 'Base Color':
                ...
                mixnode.inputs[1].default_value = pi.default_value
