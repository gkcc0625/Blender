...
class QOL_RingArray(Operator):
    ...
    #add properties
    number_of_objects : IntProperty(name="Number of objects",default=ra_prefs.count,min=1,max=1000)
    radius_of_circle : FloatProperty(name="Radius of circle",default=ra_prefs.radius,min=0.0001)
    offset_angle : FloatProperty(name="Offset angle",default=0)
    apply_transform : BoolProperty(name="Apply transform",default=ra_prefs.apply_transform)
    hub_axis : EnumProperty(name="Axis",items=[ ('X', "X", "X"),('Y', "Y", "Y"),('Z', "Z", "Z")],default = "Z",)
    delete_original : BoolProperty(name="Delete original",default=ra_prefs.delete_original)
    linked_data : BoolProperty(name="Linked data",default=ra_prefs.linked_data)
    create_parent : BoolProperty(name="Create parent",default=ra_prefs.create_parent)
    merge_objects : BoolProperty(name="Merge objects",default=ra_prefs.merge_objects)
    autoAxis : BoolProperty(name="Auto axis",default=ra_prefs.auto_axis)
    ...
    def execute(self, context):
        ...
        #check for transform option and apply if true
