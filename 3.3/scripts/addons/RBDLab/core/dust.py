...
class DUST_OT_add(Operator):
    ...
    def execute(self, context):
        ...
        if coll_name:
            deselect_all_objects()
            ps_name = coll_name + '_Dust'
            obj = get_first_mesh_visible()
            if obj:
                set_active_object(obj)

                if ps_name not in obj.particle_systems:
                    ps_type = bpy.context.scene.rbdlab_props.ps_dust_type_combobox
                    dps_size_detail = 0.025
                    lifetime = bpy.context.scene.frame_end
                    count = bpy.context.scene.rbdlab_props.dust_count
                    v_group = 'Interior'
                    normal = 2
                    randomize = 2
                    rotation = True
                    ...
                    if ps_name not in obj.particle_systems:
                        ...
                        # PROPAGATE PARTICLES:
                        if bpy.context.scene.rbdlab_props.ps_dust_from == 'BROKEN':
...
class DUST_OT_rm(Operator):
    bl_idname = "dust.rm"
    bl_label = "Remove Dust"
    bl_options = {'REGISTER', 'UNDO'}
