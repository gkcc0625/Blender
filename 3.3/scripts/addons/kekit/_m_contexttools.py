...
class UIContextToolsModule(Panel):
    ...
    def draw(self, context):
        ...
        row2.prop(bpy.context.scene.kekit, "korean", text="K/F", toggle=True)

        row = col.row(align=True)
        row.operator('mesh.ke_contextextrude')
        row2 = row.row(align=True)
        row2.alignment = "RIGHT"
        row2.prop(bpy.context.scene.kekit, "tt_extrude", text="TT", toggle=True)

        row = col.row(align=True)
        row.operator('view3d.ke_contextdelete')
        row2 = row.row(align=True)
        row2.alignment = "RIGHT"
        row2.prop(bpy.context.scene.kekit, "h_delete", text="H", toggle=True)
...
class KeContextSelect(Operator):
    ...
    def execute(self, context):
        if context.mode == "EDIT_MESH":
            ...
            fh = context.scene.kekit.context_select_h
