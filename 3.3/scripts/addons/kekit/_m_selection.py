...
class UISelectByDisplayType(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(context.scene.kekit, "sel_type_coll", toggle=True)
...
class KeSelectByDisplayType(Operator):
    ...
    def execute(self, context):
        ac = context.scene.kekit.sel_type_coll
        ac_objects = []
        ac_check = True
        if ac:
...
def register():
    if bpy.context.preferences.addons[__package__].preferences.m_selection:
        ...
        # bpy.types.Scene.kekit_cursor_obj = bpy.props.PointerProperty(type=bpy.types.Object)
