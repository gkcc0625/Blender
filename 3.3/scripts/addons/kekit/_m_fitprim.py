...
class KeFitPrim(Operator):
    ...
    def invoke(self, context, event):
        self.screen_x = int(bpy.context.region.width * 0.5)
    ...
    def execute(self, context):
        ...
        if context.scene.kekit.fitprim_item:
            self.itemize = True
        else:
            self.itemize = self.ke_fitprim_itemize

        self.modal = context.scene.kekit.fitprim_modal
        self.cyl_sides = context.scene.kekit.fitprim_sides
        self.select = context.scene.kekit.fitprim_select
        self.unit = round(context.scene.kekit.fitprim_unit, 4)
        self.sphere_ring = context.scene.kekit.fitprim_sphere_ring
        self.sphere_seg = context.scene.kekit.fitprim_sphere_seg
        self.quadsphere_seg = context.scene.kekit.fitprim_quadsphere_seg
        ...
        if not sel_verts or self.edit_mode == "OBJECT":
            ...
            # todo: (Linux only?) "pie menu op refuses to grab event data in invoke or some shit" bug? temp-workaround:
            if self.screen_x == 0:
                print("FitPrim: Pie-Invoke/Event Bug. Congratulations. Fallback: Obj center loc used for mousepos.")
...
def register():
    if bpy.context.preferences.addons[__package__].preferences.m_fitprim:
        try:
            if not bpy.context.preferences.addons[__package__].preferences.m_modeling:
                UIFitPrimModule.bl_category = __package__
                UIFitPrimModule.bl_parent_id = ""
            else:
                UIFitPrimModule.bl_category = ""
                UIFitPrimModule.bl_parent_id = "UI_PT_M_MODELING"
...
def unregister():
    if "bl_rna" in UIFitPrimModule.__dict__:
