...
class KePieSubd(Menu):
    ...
    def poll(cls, context):
        return context.space_data.type == "VIEW_3D" and context.active_object
    ...
    def draw(self, context):
        ...
        if subd_mods:
            for m in subd_mods:
                ...
                s.prop(m, "use_creases", toggle=False)
        ...
        if vertex_groups:
            ...
            for group in vertex_groups:
                ...
                if n[:3] == "V_G" and n not in used:
                    b.operator("ke.pieops", text="Add " + group.name + " Bevel", icon="MOD_BEVEL").op = "VG_BEVEL¤" + n
                    b.separator(factor=0.7)
        ...
        split.prop(active.data, "use_auto_smooth", text="AutoSmooth", toggle=False)
        ...
        if op_is_registered("VIEW3D_OT_ke_solo_cutter"):
            ...
            row.enabled = False
            row.label(text="Solo Cutter N/A")
        spacer = m.column()
        spacer.label(text="")
...
class KePieSnapping(Menu):
    ...
    def draw(self, context):
        ...
        name1 = context.scene.kekit.snap_name1
        name2 = context.scene.kekit.snap_name2
        name3 = context.scene.kekit.snap_name3
        name4 = context.scene.kekit.snap_name4
        name5 = context.scene.kekit.snap_name5
        name6 = context.scene.kekit.snap_name6
        ...
        cbox.prop(context.scene.kekit, "combo_autosnap", text="", toggle=True)
...
class KePieOverlays(Menu):
    ...
    def draw(self, context):
        ...
        if o.show_extra_indices:
            ...
            cbox.operator("view3d.ke_overlays", text="Indices", icon="LINENUMBERS_ON",
