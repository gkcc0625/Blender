...
def f_BuildSettings(self, context):
    ...
    if cTB.vSettings["show_default_prefs"]:
        ...
        vLodCol.label(text="Default Model LOD :")
        ...
        for vL in ["SOURCE", "LOD0", "LOD1", "LOD2", "LOD3", "LOD4"]:
            vOp = vPGrid.operator(
                "poliigon.poliigon_setting",
                text=vL,
        ...
        for vS in ["1K", "2K", "3K", "4K", "6K", "8K", "16K"]:
            vOp = vPGrid.operator(
                "poliigon.poliigon_setting",
                text=vS,
                depress=(vS in cTB.vSettings["hdrib"]),
            )
        ...
        for vS in ["1K", "2K", "3K", "4K", "6K", "8K", "16K"]:
            vOp = vPGrid.operator(
                "poliigon.poliigon_setting",
                text=vS,
                depress=(vS in cTB.vSettings["hdri"]),
            )
    ...
    if environ.env and "prod" not in environ.env.lower():
        self.layout.alert = True
        msg = f"Active environment: {environ.env}, API: {environ.api_url}"
